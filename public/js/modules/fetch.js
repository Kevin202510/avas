...
const update1 = async (_entity, pk, params, withMsge = true) => {
    let url = `/${_entity.baseUrl}/${pluralize(_entity.name)}/${pk}/update1`;
    const model = await request(url, params, "PUT");
    if (model) {
        if (withMsge) {
            swal.fire({
...
const update2 = async (_entity, pk, params,func, withMsge = true) => {
    let url = `/${_entity.baseUrl}/${pluralize(_entity.name)}/${pk}/${func}`;
    const model = await request(url, params, "PUT");
    if (model) {
        if (withMsge) {
            swal.fire({
...
const updateVote = async (_entity, pk, params, withMsge = true) => {
    let url = `/${_entity.baseUrl}/${pluralize("candidate")}/${pk}/updateVoteCount`;
    const model = await request(url, params, "PUT");
    if (model) {
        // if (withMsge) {
        //     swal.fire({
...
const translates = async (_entity, key = "") => {
    entity = _entity.name;
    baseUrl = _entity.baseUrl;
    $("#table-main").empty();
    let params = key;
    ...
    const _models = await ask(`/${baseUrl}/${pluralize(entity)}/${key}`);
    return _models;
};
...
const writer = (_entity, model) => {
    ...
    if(_entity.name != "student" && _entity.name != "voter"){
            ...
            if (typeof val === "boolean") {
                if (model[actions[key][3][0]] == val) {
                    $("<button>", {
                        "data-index": index,
                        "data-id": model.id,
                        class: `btn btn-${actions[key][2]} btn-${key} btn-sm`,
                        "data-toggle": "tooltip",
                        title: actions[key][1],
